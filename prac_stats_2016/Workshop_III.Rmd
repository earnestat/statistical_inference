...
**Workshop III** is concerned primarily with *model parameter estimation* (*model fitting*) and *model selection*. We first introduce maximum likelihood estimation (MLE), as this is a default estimation method for many working scientists and softwares. We provide worked theoretical example and worked experimental examples from the recent cell biological literature. We provide an intuition towards model selection and an example of a model selection criterion, the Akaike Information Criterion. Although this is NOT a Workshop on mathematical modeling, but rather one on parameter fitting and model selection, we provide a discussion of how to think about such modeling. We then turn our attention towards simple linear regression, which is in fact a special form of MLE! We provide examples, along with an in depth discussion of what assumptions underlie linear regression and when you should and should not use it. We discuss bootstrapping to obtain confidence intervals on parameter estimates. We generalize this discussion to modeling nonlinear relationships utilizing nonlinear least squares. We promote viewing this landscape of data analytic techniques as a nested sequence:
...
Let us say that we want to figure out the proportion of cases in which a mutation has a particular effect: for example, how often a mutant dies. 
...
If, on the other hand, $P(H)=0.75$, $1.96\times\text{SD}\approx 1/\sqrt{4n/3}$ and to get $95\%$ confidence intervals of the same size, we only require 300 tosses of the coin.
...
**Essential knowledge**: if there are $n$ parameters $p_1,\ldots,p_n$ in a model and the data $D = \{D_i\}$ are independent, then the likelihood function is

$$\mathcal{L}(p_1,\ldots,p_n|D) = \prod_i P(D_i|p_1,\ldots,p_n)$$
...
and this is the function that we minimize in R as shown below, after this note.
...
Gardner and co-authors hypothesized that lifetimes may actually follow gamma distributions, which are thought of as resulting from multi-step processes.

```{r , fig.width = 6 , fig.height = 3 , message = FALSE , echo=TRUE , warning=FALSE}
#a plot of typical exponential & gamma distributions
#generate vector for x-axis -- seq() is a great function so remember it:
x <- seq(0,10, by = 0.1) 
...
**Aim**: We want to determine whether tubulin lifetimes are exponentially or gamma distributed. In order to do so, we proceed in 2 steps:
...
LL_gamma <- function( shape , rate ){
  #gamma distribution --note log = TRUE computes the log:
  R <- dgamma( x , shape , rate , log = TRUE)
  -sum(R) #-ve log likelihood
}
#perform the fit:
fit_gamma <- mle( LL_gamma , start = list( shape = 1 ,rate = 1) , 
...
exp_fit <- pexp(xax , rate =coef(fit_exp)) #exponential CDF for y-axis
 #gamma CDF for y-axis:
gam_fit <- pgamma(xax , shape = coef(fit_gamma)[1] , rate =coef(fit_gamma)[2])
...
#now we plot it all:
p + geom_line() + geom_line(aes(y=exp_fit, colour = "Exponential")  , label = "1") + 
  geom_line(aes(y=gam_fit, colour = "Gamma") ) +
...
How do you know to fit an exponential distribution and then to try a gamma distribution? What are the rules of thumb for modeling this type of data? Unfortunately, this Workshop is NOT about mathematical modeling, an incredibly rich super-field of its own. However, let me say a few things:
...
```{r , fig.width = 4 , fig.height = 3 , message = FALSE , echo=TRUE , warning=FALSE}
file <- file.path("data" , "lifetimes_mcak.csv")
data <- read.csv( file ) #load data from .csv
x <- data$X9_nM_MCAK #choose column
x <- x[!is.na(x)] #remove NAs
LL_gamma <- function( shape , rate ){
  #gamma distribution -- log = TRUE already logs it
  R <- dgamma( x , shape , rate , log = TRUE)
  -sum(R) #-ve log likelihood
}
#perform fit
fit_gamma_mcak <- mle( LL_gamma , start = list( shape = 1 ,rate = 1) , 
                       method = "L-BFGS-B" , lower = 0.00001)
...
#y-axis:
xex <- dgamma(xx , shape = coef(fit_gamma_mcak)[1] , rate =coef(fit_gamma_mcak)[2])
df1 <- data.frame( "x" = xx , "y" = xex ) #dataframe
p <- ggplot( df , aes( x = x))  #initiate plotting structure for data density
#plot density and add layer of fit on top:
p +  geom_histogram(aes(y = ..density..) , binwidth = 75) +
...
**To the eye**, the gamma fit looks better than the exponential fit: but is it? Let's direct our attention now to quantitative methods for model selection:
...
To quote Wikipedia, *the preferred model is the one with the minimum AIC value.* Hence AIC rewards goodness of fit (as assessed by the likelihood function), but it also includes a penalty that is an increasing function of the number of estimated parameters.

**Essential note**: the AIC is NOT a statistical test that tells you how good a fit is, it is a way to compare a number of models.
...
1. You can use other criteria, such as Bayesian information criterion $\text{BIC} = -2\text{ln}(L) + 2K\text{ln}(n)$. BIC will generally penalize models more heavily than AIC for complexity. They should generally agree as to which model they select and, if not, this would be the time to consult a mathematical modeler and/or a statistician.
...
The challenge: we have data $D=\{x_i,y_i\}$ consisting of two variables that seem to be linear with some noise:
...
#plot data w/ linear fit & 95% confidence bands:
p + geom_point() + stat_smooth( method = "lm" )
```
...
```{r , fig.width = 4 , fig.height = 3 , message = FALSE , echo=TRUE}
file <- file.path("data" , "spindle_size_MG.csv")
df <- read.csv( file ) #load data from .csv
...
**An absurdly concise description of the biological system**: Matt Good & co-authors ([Good et al., 2013](http://www.ncbi.nlm.nih.gov/pubmed/24233724)) put *Xenopus* egg extracts in different sized compartments to see whether spindle size was a function of compartment size. See the paper for more details: it's a great system! 
...
and the OLS method for estimating $a,b$ is to find the parameters $\hat{a},\hat{b}$ that minimize $SS_{red}$. 

**Notes:** (i) in this case of simple (one independent variable) linear regression, there is an analytic solution for the estimates: $\hat{b} = r_{xy}\times\frac{\text{SD}_y}{\text{SD}_x}$ and $\hat{a} = \bar{y} - \hat{b}\bar{x}$, where $r_{xy}$ is the sample correlation coefficient between the $x_i$ and $y_i$; (ii) In more complex situations of *least-squares curve-fitting*, minimization is performed via an algorithm called *gradient descent*: if you're interested, check it out.

**Key question**: can we get error bars on these estimates? The answer is yes and wait to see how. R will readily plot the OLS estimate for you, along with $95\%$ confidence bands.
...
What would the residuals look like here? Check them out:
...
4. Perform steps 2-3 above $n$ (commonly $=1000$) times. Then we have $n$ estimates for $a,b$, giving us a distribution, and we can look at the statistics of these distributions, such as the standard deviation, variance and confidence intervals.
...
This is due to our basic log laws: (i) $\text{log}(ab) = \text{log}(a) + \text{log}(b)$ & (ii) $\text{log}(a^b) = b\text{log}(a)$. It follows from these that if two variables are related by a power law $y=ax^n$, then their logs are related linearly:
...
```{r , fig.width = 3 , fig.height = 3 , message = FALSE , echo=FALSE ,warning=FALSE}
df2 <- df1
df2$logx <- log(df2$x)
df2$logy <- log(df2$y)
...
Looking at the residuals we see that, in this case, the model wasn't great as the residuals are heteroscedastic. Instead of fitting a line to the log-transformed data, we should then try to fit a nonlinear curve (power law) to the non-transformed data.
...
ggplot(data=df1, aes( x , y)) + geom_point() + 
    geom_smooth(method="nls", formula = y ~ x**n, start=list(n = n1) , 
                se=F ,control = list(maxiter = 500), size = 1) 
```
...
- White, C. R., Seymour, R. S. 2003. *Mammalian basal metabolic rate is pro- portional to body mass2/3.* Proceedings of the National Academy of Sciences, vol. 100, no. 7, pp. 4046–4049.