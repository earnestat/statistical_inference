---
title: "Practical Statistics Workshop II: statistical hypothesis testing"
output:
  pdf_document:
    toc: true
    fig_caption: false
...
- **To say *anything* about the population mean**, make at least $n = 6$ distinct measurements and preferably $n=10$: the reason for 6 is that this is the minimum number of observations necessary to determine whether something is statistically significant at the 95% confidence level. If you toss a (fair) coin 6 times, then there is a 1/32 chance (~3%) that it is the same each time. In other words, if you toss a coin 6 times and it is always heads or tails then you can conclude at the 95% confidence level (actually 97%) that the coin is not fair; what you really want to do however, is to look at your sample mean, along with your standard error, after these initial measurements and move from there;
...
This question is of interest for many reasons, including the fact that many statistical tests, such as the Student's t-test, make assumptions on the data in question, one of which is that it is normally distributed.
...
path <- file.path("~" , "repos" , "statistical_inference" , "prac_stats_2016")#path for working directory
...
source( file )
file <- file.path("scripts" , "qqplot.R")
source( file )
...
The Lilliefors test for normality tests whether any given data set is statistically different from a normal distribution. It is a particular case of the Kolmogorov-Smirnov test (we won't cover this here but do check it out because it is one of the great statistical tests!). Given a data set $X =\{ x_i \}$, of size $n$, we wish to test the **null hypothesis** 

$$H_0: X \text{ is drawn from a normal distribution.}$$

Then the **alternative hypothesis** is
$$H_1: X \text{ is NOT drawn from a normal distribution.}$$ 

To test the **null hypothesis**,

1. We compare the cumulative distribution functions of $X$ and the normal distribution with mean $\mu_X$ & variance $\sigma_X^2$. To do so we calculate the largest vertical distance $d_{max}$ between them;
...
4. If $p<\alpha,$ we can conclude with $(1-\alpha)$ confidence that $X$ was not drawn from a normal distribution. Classically, it is required that $\alpha = 0.05$, in which case we can conclude with 95% confidence that $X$ was not drawn from a normal distribution.

Here we plot the empirical CDFs of a Cauchy and a Gaussian distribution: can you tell why which is which (as labelled)? Once could also plot each separately against a theoretical Gaussian in order to visually make sense of the distance $d_{max}$.
...
lillie.test(x) #Lilliefors test for data set x
lillie.test(y) #Lilliefors test for data set y
```
...
Using one of the following data sets loaded/generated in the code chunks below, do the following (data set 1: house fly wing lengths; data set 2: previously generated data that we claimed to be *Gaussian* in order to demonstrate the Central Limit Theorem):
...
file <- file.path("data" , "house_fly_wing_lengths.txt")
data <-read.table( file ) #read data from .txt
```
...
yo  <- ifelse( t < 2 & t > -2, t , 0 )
#yo  <- ifelse( t < -1 , t , 0 )
df$yo <- yo
ggplot( df , aes( x=t , y=y )) + 
  geom_line()+
  geom_area(mapping = aes( y=y ) , fill = "red") +
  geom_area(mapping = aes( yo ) , fill = "white") +
  scale_y_continuous(limits = c(0, 0.4)) +
  #scale_x_continuous(limits = c(-2, 2))  +
  xlab("t") +
  ylab("Probability (t-distribution)")
```
...
I have also shaded in blue the effect sizes that are not statistically significant at the $95$% confidence level.
...
ggplot( df , aes( x=t , y=y )) + 
  geom_line()+
  geom_area(mapping = aes( yo ) , fill = "blue") +
  scale_y_continuous(limits = c(0, 0.4)) +
  scale_x_continuous(limits = c(-2, 2))  +
  xlab("Effect size (measured in standard deviations away from the 
       (null) hypothesized mean)") +
  ylab("Probability (t-distribution)")
```

**Note**: This is a two-tailed t-test. A one-tailed t-test would, for example, have as the alternative hypothesis $H_1 : \mu > \mu_0$ and it is relatively rare. One-tailed t-tests are usually used when you have strong prior knowledge that $\mu \geq \mu_0$, for example, when the measurement is of a quantity that is $\geq 0$ (such as amount of protein in a fluorospectrometer) and $\mu_0 = 0$.

**To keep in mind**: For a t-test, the magic number is 2: for a large number of degrees of freedom (number of data points $n>30$), the t-distribution is the same as a normal distribution and for a normal distribution 5% of the outcomes are more than 1.96 $\approx$ 2 standard deviations away from the mean.
...
**Answer**: at the $95\%$ confidence level, we require that $|T|>2$ (actually a bit more as $n=16$ and the t-distribution is not quite a normal distribution); but 
...
1. Due to the Central Limit Theorem, the t-test does NOT require your data to be normally distributed. A general rule of thumb is that if your sample size $n>30$, then a t-test is appropriate (unless your data is highly skewed; for more, see [here](https://onlinecourses.science.psu.edu/stat414/node/261); also check out [(Lumley et al., 2002, *The importance of the normality assumption in large public health data sets*)](http://www.ncbi.nlm.nih.gov/pubmed/11910059)).
...
In the one-sample t-test above, we tested whether there was a statistically significant effect when comparing an experiment to a known quantity $\mu_0$. Most of the time, we won't know  $\mu_0$, but we will have another data set from a control experiment.
...
2. Perform t-tests for a variety of pairs of data sets at differing [Tb].
...
Also check out the Nature Methods series [*Points of Significance*](http://www.nature.com/collections/qghhqm/pointsofsignificance) from their [*Statistics for Biologists* web collection](http://www.nature.com/collections/qghhqm/), in particular, their papers on [designing comparative experiments](http://www.nature.com/nmeth/journal/v11/n6/full/nmeth.2974.html) & [analysis of variance and blocking](http://www.nature.com/nmeth/journal/v11/n7/full/nmeth.3005.html).
...
**Motivating example**: Plotted below is the 'age at death from 3 different classes of society of Europeans' (data from [here](http://www.stat.ufl.edu/~winner/data sets.html)):

```{r, fig.width = 4 , fig.height = 3 , message = FALSE , echo=FALSE}
#data from here:
#http://www.stat.ufl.edu/~winner/data sets.html
file <- file.path("data" , "agedeath.dat")
...
**Question**: The sample means of the groups are different but *is this difference statistically significant*? In other words, could the *between-group* variance arise statistically from the variance *within groups*? ANOVA provides a powerful method to answer such questions.
...
**Intution**: The sample means $\bar{M}_i$ will be different. The real question is: could the *between-group* variance statistically arise purely from the variance *within groups*?
...
**Question:** When do we use a t-test & when do we use ANOVA?

**Answer:** One of the major strengths of ANOVA, as opposed to t-tests, is that when you are making a large number of comparisons between very many groups, ANOVA controls for *Type I errors*, whereas you need to be very careful in performing corrections yourself with a t-test. A strength of t-tests is that they tell you which groups are different from each other so, if it is necessary to know this, you may wish to conduct corrected t-tests. You also may wish to consult a statistician as this is very context-dependent!

##Example 2: basic two-way ANOVA for gene expression levels (bioinoformatics)

We will consider the example from (Pavladis, 2003),
*Using ANOVA for gene selection from microarray studies of the nervous system* (a great paper to read when thinking about experimental design and statistical interpretations of results).

In such a two-way ANOVA, the expression level of a gene is given by

$$E_{ijk} = T_i + S_j + (T\cdot S)_{ij} + \varepsilon_{ijk},$$

Here $T$ & $S$ are factors (categorical variables), and in (Pavladis, 2003), $T$ consists of 6 regions of the brain and $S$ consists of 2 levels that are different mouse strains (this is what is meant by *two-way*, that we have two variables, $T$ & $S$). This is a generalization of the one-way ANOVA set-up above (which was one factor, societal class,  with three levels: aristocracy, gentry, sovereignty). For example, the expression level in region 3 of strain 1 is given by
...
file <- file.path("data" , "sandberg-sampledata.txt")
...
6. Write all the p-values to a .txt and save it for later use; in the code chunk here I have saved the p-values to a data frame: all you need to do is write it to a .txt;
```{r, fig.width = 8 , fig.height = 6 , message = FALSE , echo=TRUE , eval = FALSE}
anova_res <- lapply( anovaresults , anova) #this will compute the ANOVA table from
#anovaresults
pvalues<-data.frame( lapply(anova_res, function(x) { x["Pr(>F)"][1:3,] }) )#save p-values
#to  a data frame
```
7. A common question in bioinformatics is to see which genes' expression levels are most affected by, for example, the region in the brain we are looking at. To this end, rank the genes according to the *main region effect* term $T$.

##Example 3: testing for multimodality
...
1. Bin the data;
2. Compute the variance $Var_X$ of the data;
3. For each bin (indexed by $i$), divide the entire data set into two sub-population $X_{i1},X_{i2}$, separated by the left end point of the bin. Compute the mean variance $Var_i = (Var(X_{i1}) + Var(X_{i2}))/2$;
4. Pick the decomposition of $X$ that minimizes $Var_i$;
5. Compute the F-statistic $F=\text{min}_i(Var_i)/Var_X$;
...
- Box, Hunter & Hunter. 1978. *Statistics for Experimenters.* *Wiley & Sons*.
- Cumming, G. 2011. *Understanding The New Statistics: Effect Sizes, Confidence Intervals, and Meta-Analysis* *Routledge*.
- Cumming, G., 2013. *The New Statistics: Why & How.* Psychological Science.
- Downey, A. 2011. *Think Stats.* *O'Reilly Media* ([.pdf here](http://greenteapress.com/thinkstats/)).
...
- Howard, Joe. *Statistical Inference and Models* notes for MB&B 435/635/ENAS518 - MATHEMATICAL METHODS IN BIOPHYSICS at Yale University, Fall 2013 (not publicly available).
...
- Pavladis, P. 2003 *Using ANOVA for gene selection from microarray studies of the nervous system.* Methods 31(4):282-9.
