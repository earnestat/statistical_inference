...
We have seen, when fitting models and estimating parameters, that we can retrieve mean estimates and confidence intervals on these estimates. But can we get the distributions? We can and one method to do so occurs in Bayesian inference.

#Bayes' Theorem

$$P(A|B) = \frac{P(B|A)P(A)}{P(B)}$$

Insert proof here, Hugo.
...
```
2. The likelihood function is given by $L = p^k(1-p)^{n-k}$ -- fill in the function in the code below (multiplication in R is $*$  and 'to the power' $**$):
...
k <- 
LL <- likelihood(n,k)
#compute the posteriors here and normalize them (by dividing by the maximum)
post1 <- 
post2 <- 
