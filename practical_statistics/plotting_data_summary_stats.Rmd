...
qplot( data$`12 uM` , xlab = "MT lifetime at 12uM") #plot a histogram of the data at 12uM
```
...
colnames(data)
qplot( dataM$`9 nM MCAK` , binwidth = 100 ,xlab = "MT lifetime at 9 nm MCAK") #plot a histogram of the data at 12uM
```
...
```{r , fig.width = 4 , fig.height = 3 , message = FALSE}
ggplot(data, aes(x = `12 uM`)) + stat_ecdf() + xlab("Microtubule lifetime")#plot ECDF of the data
```

And now we'll plot the ecdf for data that we generate:
