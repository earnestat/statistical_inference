...
**A little bit of biological background:** Microtubules are cytoskeletal polymers that alternate between periods of slow growth and rapid shrinkage. The conversion from the *growing* state to the *shrinking* state is called microtubule *catastrophe* and the time it takes from nucleation to catatrophe is called the microtubule's *lifetime*. Gardner & co-authors used a TIRF assay (we won't go into details of what this is here) to measure microtubule lifetime *in vitro* under 3 conditions: i) in the presence of tubulin alone (well, with buffer + ATP), ii) in the presence of tubulin and the microtubule-associated protein (MAP) Kip3 & iii) in the presence of tubulin and the MAP MCAK.  In i), the authors perform the assay at different tubulin concentrations and in ii) & iii) at different concentrations of the relevant MAP. The idea of the paper is to see how different MAP's effect the lifetime of microtubules.
...
**How we will store & visualize the data in R**: We will use a visualization library called **ggplot2** -- there is a bit of a learning curve in figuring out how to use it but once you've got it going in, it can be used to create beautiful figures with ease. It is based around Leland Wilkinson's [*Grammar of Graphics*](https://www.springer.com/us/book/9780387245447) and many publications, such as the [New York Times](http://www.nytimes.com/interactive/sports/football/2013-fantasy-football-tier-charts-QB.html), use it. In order to use ggplot2 to its fullest capabilities, we will store our data as **dataframe** objects -- we will play around with these a great deal in these Workshops -- all you need to know now is that '[a] data frame is a list of variables of the same number of rows with unique row names' according to R's documentation (if this is still opaque, we'll figure out below what a dataframe is by looking at one!).

```{r qplot , fig.width = 4 , fig.height = 3 , message = FALSE}
library( ggplot2 ) #load ggplot2 library for handsome plotting structures
setwd("~/repos/statistical_inference/practical_statistics/")#You need to set your working directory
...
2. Look at the variable 'data' (you can do this by executing the command 'View(data)' in the console): what does it look like? This is called a *dataframe* and will be one of your best friends when performing data analysis in R; 
...
- One can also define binwidth instead of the number of bins.
...
- CDFs are a great way to visualize and compare distributions as there is no binning and thus no artefacts introduced by binning!
- A word of warning: there is correlation in the curve (as opposed to a histogram): All that I'm saying here is that if there is a large fluctuation for a low value, then this flucutation may be seen throughout the rest of the curve. it is important to keep this in mind.
...
We now plot the microtubule lifetime data as a set of box-plots (to be defined below) with [Tb] on the x-axis.

However, to do so, we want to have the microtubule lifetime data in a different form -- we would like a dataframe in which each row is a single experiment with 2 columns: (i) the recorded lifetime & (ii) the tubulin concentration of the experiment. There is a great package 'reshape2' that helps you reshape dataframes: two important function are 'melt' and 'dcast', which are analogues of melting and cast metal -- for more information see [here](http://seananderson.ca/2013/10/19/reshape.html).

```{r,  message = FALSE ,  echo = TRUE}
library( reshape2 ) #this library contains the function that we'll need in order to reshape the dataframe
#as desired
...
##Fourth way to plot (univariate) data: column scatter plot (with jitter!)

For small amounts of data, you should plot all the data, not just the box-plot, which summarizes aspeects of the data. A column scatter plot (with jitter) is one way to achieve this and you can even layer it on top of a box-plot:
...
**Exercise (~10 minutes)**: create a beeswarm plot of the microtubule lifetime data with (i) tubulin concentration on the x-axis & (ii) lifetime on the y-axis.

**Question**: Is there the correct amount of data for a beeswarm plot here?

**Pros & Cons of beeswarm plots**:

- Bee swarm plots are a great way to visulaize all of your data if there is too much of it for a scatter plot;
- You can have too much data for a bee swarm, in which case you'll want to use a box-plot, for example.
...
- Radioactive decay;
- Any Poisson Process has exponentially distributed waiting times (if you don't know what this means, that's fine! You will soon);
...
**Exercise (~5 minutes)**: Figure out what each line of the code above does.
...
1. Here we perform a simple demonstration of the Central Limit Theorem, hi which our random variable defined by $p(0)=p(1)=0.5$:
...
2. This example will take the form of an **exercise (~15minutes)**: choose one the microtubule lifetime datasets and compute the mean of a random subset of it; do this 10,000 times and plot the resulting histogram;

3. In which we  estimate $\pi$ using a Monte Carlo method:
...
*Bayesian Logical Data Analysis for the Physical Sciences* Phil Gregory.

R cheat sheets: https://www.rstudio.com/resources/cheatsheets/

#Code for Solutions to Exercises
...
```{r,  message = FALSE ,  echo = TRUE}
datamm <- read.csv("mm.csv")
by(datamm$Speed , datamm$Expt , mean)
```
There is another supercool way in which we reshape (cast) the data and then use 'apply':
```{r,  message = FALSE ,  echo = TRUE}
d <- dcast(datamm , Run ~ Expt , value.var = "Speed")
apply( d[,2:6] , 2 , mean)
```

**Exercise (~10 minutes)**: Compute the standard deviation of one of the microtubule lifetime datasets (for all concentrations) OR the the Michelson-Morley data (all experiments).
...
2. This example will take the form of an **exercise**: choose one the microtubule lifetime datasets and compute the mean of a random subset of it; do this 10,000 times and plot the resulting histogram;

**Code for solution**:
```{r, fig.width = 4 , fig.height = 3 ,  message = FALSE ,  echo = TRUE}
data <- read.csv("MG_catastrophe_data/lifetimes_tubulin.csv" , check.names=FALSE) #load data from .csv
d12 <- data$`12_uM`
samples <- replicate(10000,sample( d12 , 300,replace = TRUE))
