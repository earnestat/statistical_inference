...
Let's say that we want figure out the proportion of cases in which a mutation has a particular effect: for example, how often does a mutant die. 

<span style="color:red">Give more simple examples of when using binomial is a good idea. </span>
...
An intuitive **answer** to the 1st question is $\hat{p} = k/n$, the number of heads divided by the total number of coin tosses. We will now see why:
...
**Computational Note**: Because the likelihood is usuallly very large, instead of maximizing the likelihood, we usually minimize the negative log-likelihood $-\mathcal{LL}$ (more on this below).
...
<span style="color:red">**Note**: I would really like to see here a (short) discussion of the intuition that our confidence is related to the variance of the result when performing the experiment many times -- then the participants could simulate it themselves and make their own error bars! </span>
...
**Essential knowledge**: if there are $n$ parameters $p_1,\ldots,p_n$ in a model, then the negative log-likelihood

$$-\mathcal{LL}(p_1,\ldots,p_n|D) = -\sum \text{log}P(D_i|p_1,\ldots,p_n)$$

and this is the function that we minimize in R:
...
fit_exp <- mle( LL_exp , start = list( rate = 1) )
summary(fit_exp)
```
...
xax <- seq(0,1600 , by = 1)
ec <- P(xax)
exp_fit <- pexp(xax , rate =coef(fit_exp)) #y-axis exp
gam_fit <- pgamma(xax , shape = coef(fit_gamma)[1] , rate =coef(fit_gamma)[2]) #y-axis gamma
df <- data.frame( "x" = xax , "ecdf" = ec , "exponential" = exp_fit , "gamma" = gam_fit)
p <- ggplot( df , aes(x=x , y = ecdf , colour = "ECDF"))
...
There are many ways to select the best model: we want one that rewards models that fit well BUT that also penalizes models with many free parameters (in the spirit of Occam's razor & due to overfitting). The Akaike information criterion is an example of such a measure.
...
**Note**: Can use other criteria, such as Bayesian information criterion (BIC).

Also, see here: http://brianomeara.info/tutorials/aic/

Also, do see this: http://www.r-bloggers.com/fitting-a-model-by-maximum-likelihood/
...
Performing linear regression in R in super-straightforward:
...
```{r , fig.width = 4 , fig.height = 3 , message = FALSE , echo=TRUE}
df <- read.csv("spindle_size_MG.csv")
p <- ggplot( df , aes(x = Droplet_Diameter_.µm. , y = Spindle_Length_.µm.))
p + geom_point(alpha = 0.5)
```

**The biological system**: Matt Good & co-authors put *Xenopus* egg extracts in different sized compartments to see whether spindle size was a function of compartment size. 
...
```{r , fig.width = 4 , fig.height = 3 , message = FALSE , echo=TRUE}
df1 <- subset( df , Droplet_Diameter_.µm. <80)
```
3. Plot the data with a linear fit;
```{r , fig.width = 4 , fig.height = 3 , message = FALSE , echo=TRUE}
p <- ggplot( df1 , aes(x = Droplet_Diameter_.µm. , y = Spindle_Length_.µm.))
p + geom_point(alpha = 0.5) + stat_smooth( method = "lm") + xlim(20 , 80) + ylim( 20 , 50)
```
...
Anscombe's Quartet consists of four datasets, each consisting of 11 data points. Each data set has the same $\bar{x}, \bar{y}, \text{SD}_x , \text{SD}_y$, correlation coefficient and linear regression line (OLS):
...
```{r , fig.width = 4 , fig.height = 3 , message = FALSE , echo=TRUE}
lin_fit <- lm( Spindle_Length_.µm. ~ Droplet_Diameter_.µm. , df1)
pres <- ggplot( lin_fit , aes( .fitted , .resid) )
pres + geom_point()
```

**Exercise 5:** Discuss the result.
...
print(results)
```
<span style="color:red">**Exercise**: You do a bootstrap. Also, how about weighted least squares?? See here and follow the trail...http://www.itl.nist.gov/div898/handbook/pmd/section4/pmd452.htm#uw & https://onlinecourses.science.psu.edu/stat501/node/352</span>

#Nonlinear least squares

##Example: Modeling a power law

Power laws $y = x^n$ occur all throughout biology: see (Kleiber, 1932; Gillooly et al., 2001; White et al., 2003) for examples; they are also the primary mode of thinking about scaling in biology, which is currently very sexy!
Here we synthesize an example in which $n=0.55$:
...
2. Use the AIC to compare the linear model with the power law model. **Note**: in this case, even if the AIC reports the power law model to be better, we need to think hard about which we accept because the linear model is a mechanistic/physicalist model.
