...
<span style="color:red">Explain this equation and the notatation. </span>
...
**Computational Note**: Because the likelihood is usuallly very large, instead of maximizing the likelihood, we usually minimize the negative log-likelihood (more on this below).

<span style="color:red">**Note**: Log laws here? Or at the very least state what $-LL$ is in this case. </span>

###How confident can we be of our estimate $\hat{p}$?
...
<span style="color:red">**Note**: I would really like to see here a (short) discussion of the intuition that our confidence is realted to the variance of the result when perofrming the experiment many times -- then the participants could simulate it themselves and make their own error bars! </span>
...
x <- rbinom( m , n , p )/n # sample distribution and return vector of means
...
Do these follow exponential OR gamma distributions? Explain what there are here and give an intuition towards (log) likelihood.

Also, some of what follows here should be exercises.

<h4>Exponential distribution for tubulin alone</h4>
...
<h4>Gamma distribution for tubulin alone</h4>
```{r , fig.width = 4 , fig.height = 3 , message = FALSE , echo=TRUE , warning=FALSE}
data <- read.csv("MG_catastrophe_data/lifetimes_tubulin.csv" , check.names=FALSE) #load data from #.csv
x <- data$`12_uM`
...
<h4>Exponential distribution for tubulin & MCAK</h4>
...
fit1 <- mle( LL_exp , start = list( rate = 1) )
summary(fit1)
#confint(fit)
```


<h4>Gamma distribution for tubulin & MCAK</h4>
...
fit2 <- mle( LL_gamma , start = list( shape = 1 ,rate = 1) , method = "L-BFGS-B" , lower = 0.00001)
summary(fit2)
```

**YOU NEED TO PROVIDE PLOTS OF THESE MODELS IN ORDER TO MOTIVATE THE CHALLENGE OF MODEL SELECTION:**

#Model Selection

Many ways to select the best model: we want one that rewards models that fit well BUT that also penalizes models with many free parameters (in the spirit of Occam's razor & due to overfitting). The Akaike information criterion is an example of such a measure.
...
**Exercise**: Use AIC to compare exponential and gamma models of MT catastrophe.
...
<span style="color:red">**Example (anything) & Exercise (spindle size?)**: Perform linear regression & see how easy it is! Without knowing anything you cant do it. Now let's see what you actually did. Wanna have polynomial regression also??</span>
...
<span style="color:red">**Exercise**: Perform linear regression on Anscome's Quartet! Maybe even get all the results? mean, sd, etc....</span>
