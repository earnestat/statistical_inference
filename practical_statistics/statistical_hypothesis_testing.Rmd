...
- To say something about the variance, have at least $n = 30$ data points: this is because the **standard error of the standard deviation** is $\approx\frac{\sigma}{\sqrt{2(n-1)}}$ (see [this paper(http://web.eecs.umich.edu/~fessler/papers/files/tr/stderr.pdf)) so that, if you have 30 data points, then your **standard error of the standard deviation** is $\approx 13%$ of $\sigma$, or an order of magnitude less than it;
...
These are incredibly generic rules!  Keep them in mind and re-evaluate them when you get some data!
...
```{r , message=FALSE , fig.width = 6 , fig.height = 3, echo=TRUE}
...
Performing the Liiliefors test in R is super-straighforward:
...
2. Plot the ECDF of the data, along with the CDF of the Gaussian distribution (with mean and standard deviation those of the data);
3. perform a Lilliefors test on the data and interpret the result.
...
Below we plot the probability of seeing any given effect size, measured in standard deviations away from the (null) hypothesized mean, given $n=16$.
...
1. Generate data from a Gaussian distribution with mean=$1$, sd=$x$, for varying $x$, and see how many points you need to find a statistically significant difference between $\bar{x}$ and a (null) hypthesized mean $0$. *Note*: It is important to realise that may see a statistically significant difference randomly. Discuss this.

2. Reseachers claim that there is [*wisdom in crowds*](http://www.diplomacy.edu/resources/books/reviews/wisdom-crowds-why-many-are-smarter-few). Let's test it! More specifically, the claim is that if you ask people to guess the number of jelly beans in a jar, the *average guess* shuold be pretty close to the *true mean*. So: each person in the room will guess the number of jelly beans. The exericise is to use a t-test to test the null hypothesis $H_0$ that the average guess is equal to $J$, the number of beans in the jar. Also discuss whether the assumptions underlying the t-test are satisfied. *Also*, in this exercise, we will type the data into R and save it as a .csv using 'writecsv'.
...
##Two sample t-test
<span style="color:red">Note: this and the next section (Paired t-tests and Blocking) are cool and actually quite important, but we will probably not have time: I think I will leave the information here in order for people to have a reference, but I will not include exercises. </span>
...
Probably no exercise here but let's see.
...
**Note**: this is a conservative test, in the sense that it is prone to rejecting the alternative hypothesis when the alternative hypothesis is true( that is, it can produce *false negatives* or *type II errors*.

**Exercise:**
Perform t-tests on all 10 pairs of the microtubule datasets (i) without the Bonferroni correction & (ii) with the Bonferroni correction.

*Hint*: type the phrase *Bonferroni Correction R* into your favourite search engine.


#ANOVA & $F-$tests

An F-test is any statistical test in which the test statistic has an F-distribution under the null hypothesis. It is most often used when comparing statistical models that have been fitted to a data set, in order to identify the model that best fits the population from which the data were sampled. (according to wikipedia)
...
Let's say that we have  $n$ groups of data $M_i=\{D_{ij}\}_{j\in J}$, drawn from populations with respective *population means* $\alpha_1 , \alpha_2 , \ldots , \alpha_n$.
...
**Intution**: The sample means $\bar{M}_i$ will be different. Th real question is: could the *between-group* variance arise statistically purely from the variance *within groups*?

**Method**: To answer this, we look at the $F-$statistic which, intutively, is 

$$F = \frac{\text{between-group variability}}{\text{within-group variability}}.$$

The precise definition of the F-statistic is formulated in the Appendix below. Intuitively, if the $F-$statistic is large, then we can be fairly certain that we can reject the null hypothesis. Formally, we perform an $F-$test (also see Appendix). To do so, assume the statistical model (necessarily linear)

$$D_{ij} = \mu + \alpha_{i} + \varepsilon_{ij},$$

where the $\varepsilon_{ij}$ are Gaussian noise terms (explain the equation further?).

**Example**: Age at death from 3 different classes of society in Victorian England:
...
aov.ex1 <- aov( age~type , data)
summary(aov.ex1)
print(model.tables(aov.ex1,"means"),digits=3)       #report the means and the number of subjects/cell
```

**Exercise:** Interpret and discuss the above output.
...
##Example 3: basic two-way ANOVA for gene expression levels (bioinoformatics)

In such a two-way ANOVA, the expression level of a gene is expressed by

$$E_{ijk} = T_i + S_j + (T\cdot S)_{ij} + \varepsilon_{ijk},$$

Here $S$ & $T$ are factors (categorical variables), and in (Pavladis, 2003), $S$ consists of 2 levels that are different mouse strains and $T$ 6 regions of the brain. This is a generalization of the one-way ANOVA set-up above (one factor with three levels). Note that it is still a linear model and has three *effect* terms: *main effects* $T$ & $S$ and an *interaction effect* $T\cdot S$. When performing ANOVA in such a study, we get three $F$-statistics and thus three $p-$values for each gene: one for each of the possible effects. This same model is fitted to all genes and then, for each effect, we can rank the genes by strength of the effect in question.

We will consider the example from (Pavladis, 2003),
*Using ANOVA for gene selection from microarray studies of the nervous system.* To quote the paper in question at some length,
...
1. Import "sandberg-sampledata.txt" as a dataframe called 'data'-- but first look at the file in a text editor and see whether there are column names (header) and/or row names: this will effect how you import it;
...
6. Write all the p-values to a .txt and save it for later use;
7. Rank the genes in order of strength of effect of the interaction term $T\cdot S$.

##Example 4: testing for multimodality.

<span style="color:red">We will probably NOT make it this far but this is a cool application of the $F-$test. </span>
...
#Code chunks for Solutions to Exercises
...
**Code for (some of) solution:**
```{r, fig.width = 4 , fig.height = 3 ,  message = FALSE ,  echo = TRUE}
qqplot.data( x ) + xlab("theoretical (Gaussian)") + ylab("sample of sample means")
lillie.test(x)
```
...
**Essential Note**: In the above you shoudld actually plot concentration on the x-axis **as a numerical variable**!!
...
t.test( subset(data$`7_uM` , !is.na(data$`7_uM`) ) , subset(data$`10_uM` , !is.na(data$`10_uM`) ) )
```
**Note:** Also check out 

```{r , message=FALSE , echo=TRUE }
pairwise.t.test(df$value, df$variable, p.adj = "none" , pool.sd = FALSE)
```
...
df <- melt(data)
pairwise.t.test(df$value, df$variable, p.adj = "none" , pool.sd = FALSE)
pairwise.t.test(df$value, df$variable, p.adj = "bonf" , pool.sd = FALSE)
```
**Exercise** (code for solution embedded)
...
df <- apply(data , 1 , add_factors)
```
4. Perform ANOVA on a single gene;
```{r, fig.width = 8 , fig.height = 6 , message = FALSE , echo=TRUE , eval = TRUE}
single_gene_anova <- aov(x ~ strain + region + strain*region, df[[1]])
summary( single_gene_anova )
```
5. Use 'apply' OR a 'for loop' to perform ANOVA on all genes!
```{r, fig.width = 8 , fig.height = 6 , message = FALSE , echo=TRUE , eval = TRUE}
anovaresults <- lapply(df, function(gene) aov(x ~ strain + region + strain*region, gene))
```
6. Write all the p-values to a .txt and save it for later use;
