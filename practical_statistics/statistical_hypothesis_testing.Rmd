...
```{r qplot, fig.width = 8 , fig.height = 3 , message = FALSE , echo=FALSE}
#code chunk here:
set.seed( 42 )
library( ggplot2 )
source("multiplot.R")
...
**Exercise**
Using one of the following datasets generated in the code chunks below, do the following (dataset 1: house fly wing lengths; dataset 2: previously generated data that we claimed to be *Gaussian* in order to demonstrate the Central Limit Theorem),
...
Perform a paired t-test on the following data (coming soon!).
...
Problem: When you are performing and comparing many experiments, there is a problem. For example, let's take the absurd case in which you have a control and then you perform an experiment that has no effect (a placebo) and you do it 20 times. We know that 5% of the time you will get a statistically significant difference at the 95% confidence level (due to statistical fluctuations): this is one in 20 of your experiments! We then need a stricter criterion for statistical significance: enter the Bonferroni Correction.
...
Let's say that we have  $n$ groups of data $D_i$, drawn from populations with respective *population means* $\alpha_1 , \alpha_2 , \ldots , \alpha_n$.
...
**Intution**: The sample means $\bar{D}_i$ will be different. Th real question is: could this *between-group* variance arise from the each *within-group* variance?
...
This is one way to perform ANOVA in R:
...
```
4. Use 'apply' OR a 'for loop' to perform ANOVA on all genes!
```{r, fig.width = 8 , fig.height = 6 , message = FALSE , echo=TRUE , eval = TRUE}
anovaresults <- lapply(df, function(gene) aov(x ~ strain + region + strain*region, gene))
```
5. Write all the p-values to a .txt and save it for later use.
