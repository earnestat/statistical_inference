...
**Exercise**
Previously, we generated some data that we claimed to be *Gaussian* in order to demonstrate the Central Limit Theorem. The code is below. The exercise is to (i) plot the Q-Q-plot, (ii) plot the ECDF of the data, along with the CDF of the Gaussian distribution with mean and standard deviation those of the data and (iii) perform a Lilliefors test on the data and interpret the result.
...
**Exercise:**
Generate data from a Gaussian distribution with mean=$1$, sd=$x$, for varying $x$, and see how many points you need to find a statistically significant difference between $\bar{x}$ and a (null) hypthesized mean $0$.
...
```
**Note:** Check out pairwise.t.test(df$value, df$variable, p.adj = "none" , pool.sd = FALSE)
