...
Now: what is statistial hypothesis testing? It is merely testing how likely an hypothesis is, given the data at hand.
...
**Note**: this is a conservative test, in the sense that it is prone to rejecting the alternative hypothesis when the alternative hypothesis is true (that is, it can produce *false negatives*, also called *type II errors*).
...
The precise definition of the F-statistic is formulated in the Appendix accompaying these notes. Intuitively, if the $F-$statistic is large, then we can be fairly certain that we can reject the null hypothesis. Formally, we perform an $F-$test (also see Appendix). To do so, we assume the statistical model
...
**Exercise (~5 minutes)**: Interpret and discuss the above output.
